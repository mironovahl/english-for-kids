  ...
  if (event.target.closest('.menu')) {
    ...
    if (!event.target.classList.contains('menu__categories')) {
      ...
      if (event.target.innerText === 'Statistics') {
        if (!document.querySelector('main').contains(document.querySelector('.button-statistics'))) { StatisticButton(); }
        CONTENT.classList.add('statistic');
      } else if (document.querySelector('main').contains(document.querySelector('.statistic-button'))) {
        const elem = document.querySelector('.statistic-button');
...
CONTENT.onclick = (event) => {
  ...
  if (event.target.classList.contains('button-sort')) {
    ...
    const ev = event.target.closest('.card');
    ev.onmouseleave = () => {
    ...
    const choice = document.querySelector('.header__text_categories').innerHTML;
    const ListCard = document.querySelector('.content').childNodes;
    for (let i = 0; i < ListCard.length; i += 1) {
      if (ListCard[i] === elem) {
        if (choice === 'Statistics') {
          Object.keys(dataS).forEach((key) => {
            dataS[key].forEach((ele) => {
              if (elem.id === ele.word) {
                const c = new Card(ele);
                c.PlayAudio();
              }
            });
          });
        } else {
          const c = new Card(dataS[choice][i]);
    ...
    const elem = event.target.closest('.card').id;
    const n = new Sound(document.querySelector('.header__text_categories').innerHTML);
    const soundList = JSON.parse(localStorage.getItem('soundList'));
    const m = localStorage.getItem('randAudio');
    if (n.Check(elem, m)) {
      const k = n.AudioChoice(soundList, count);
      count += 1;
...
HEADER.addEventListener('click', (event) => {
  if (!event.target.classList.contains('header__burger') && !event.target.closest('.header__burger') && !event.target.closest('.sidebar')) { close(); }
});
